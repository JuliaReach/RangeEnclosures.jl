...
# multivariate
function enclose_BranchandBound(f::Function, dom::IntervalBox{T, N};
                                order::Int = 10, tol::Number = 0.6) where {T, N}
    n = length(dom)
    x0 = mid(dom)
...
function branchandbound(p::TaylorN{T}, dom::IntervalBox{N, W},
                                       ϵ::Number) where{T, N, W}
    return _branchandbound(p, dom, ϵ)
end

function _branchandbound(p::Union{TaylorN{T}, Taylor1{T}},
                         dom::Union{IntervalBox{N, W}, Interval{W}},
                         ϵ::Number) where {N, T, W}
    K = 1
    Rprev = evaluate(p, dom) #Rprevious
    D1, D2 = bisect(dom)
    D = [D1, D2]
    R = [evaluate(p, D[i]) for i = 1:length(D)]
    Rnext = _Rnext(R)
    while  (Rprev.hi - Rnext.hi) >= ϵ*diam(Rnext) &&
           (Rprev.lo - Rnext.lo) >= ϵ*diam(Rnext) && (K <= 1000)
        Rprev = _Rnext(R)
